...
bool
Dictionary::sameChar(unsigned int c1, unsigned int c2)
{
	if((c1 >= 0x1000 && c1 <= 0x1200) && (c2 >= 0x1000 && c2 <= 0x1200)) {
		unsigned int oCodes[2][5];
		unsigned int iCodes[] = { c1, c2 };
	...
	unsigned short lCh1 = toLowerCase(c1);
	unsigned short lCh2 = toLowerCase(c2);
...
Dictionary::sameAsTyped(unsigned short *word, int length)
{
    ...
    while (length--) {
        if (!sameChar((unsigned int) *inputCodes, (unsigned int) *word)) { // Need to change
            return false;
        }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (completion) {
            ...
            while (currentChars[j] > 0) {
                if (sameChar(currentChars[j], c)) { // SMM
